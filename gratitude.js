/* ==========================================
   УНИВЕРСАЛЬНЫЙ МОДУЛЬ СПИСКОВ (gratitude.js)
   ========================================== */

const ListModule = {
    // Уникальный ID и заголовок для этой страницы
    config: {
        storageKey: 'GL_Data_Gratitude', // Уникальный ключ в памяти
        title: 'Благодарность',
    ...
    addItem: function() {
        ...
        if (this.editingId === 'new') {
            // Добавляем в конец, чтобы нумерация была логичной (или unshift для новых сверху)
            this.data.push({
                id: Date.now(),
                text: text,
                date: date
    ...
    render: function() {
        ...
        const styles = `
            <style>
                ...
                .ls-number { 
                    font-weight: 800; color: #5856D6; min-width: 25px; 
                    font-size: 16px; margin-right: 8px; line-height: 1.5;
                }
                .ls-content { flex: 1; }
                .ls-text { 
                    font-size: 16px; line-height: 1.5; color: #1C1C1E; 
                    white-space: pre-wrap; word-break: break-word; 
                }
                .ls-date { 
                    font-size: 11px; color: #AEAEB2; font-weight: 600; 
                    text-align: right; margin-top: 8px;
                }

                .ls-add-btn {
                    background: #5856D6; color: white; border-radius: 18px;
        ...
        let listHtml = this.data.map((item, index) => `
            <div class="ls-item" onclick="ListModule.openModal(${item.id})">
    ...
    renderModal: function() {
        ...
        return `
            <div class="ls-modal-bg" onclick="ListModule.editingId=null; ListModule.render()">
                <div class="ls-modal" onclick="event.stopPropagation()">
                    <h3 style="text-align:center; margin-top:0;">${item ? 'Правка' : 'Новая запись'}</h3>
                    ...
                    <div class="ls-input-group">
                        <label class="ls-label">ТЕКСТ</label>
                        <textarea id="list-input-field" class="ls-textarea" placeholder="${this.config.placeholder}">${item ? item.text : ''}</textarea>
                    </div>

                    <button class="ls-save" onclick="ListModule.addItem()">Сохранить</button>
                    ${item ? `<div style="text-align:center; color:#FF3B30; margin-top:15px; font-weight:700; cursor:pointer;" onclick="ListModule.deleteItem(${item.id})">Удалить</div>` : ''}
                </div>
            </div>
        `;
    },
...
export function render() { ListModule.init(); }
