...
const SkinTracker = {
    ...
    render: function() {
        ...
        const styles = `
            <style>
                .sk-container { animation: fadeIn 0.3s; padding-bottom: 120px; }
                .sk-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; }
                
                .sk-clean-banner { 
                    background: #34C759; color: white; border-radius: 25px; 
                ...
                .sk-modal-content { 
                    background: #F2F2F7; width: 100%; max-height: 95vh; 
                ...
                .sk-input { 
                    ...
                    font-size: 17px; font-family: inherit; outline: none; resize: none; 
                }
                ...
                .sk-btn-save { 
                    ...
                    padding: 18px; border-radius: 20px; font-weight: 700; font-size: 17px; margin-top: 10px; 
                }
            </style>
        `;
            ...
            <div class="sk-container">
                ...
                ` : `
                    <div class="sk-cal-card">
                        <div style="font-weight:800; margin-bottom:10px;">Прогресс за всё время</div>
    ...
    getModalHTML: function(todayStr) {
        const edit = this.editingId === 'new' ? null : this.data.find(r => r.id == this.editingId);
        return `
                    ...
                    <form id="sk-form">
                        ...
                        <div class="sk-form-group">
                            ...
                            <div style="display:flex; gap:8px; flex-wrap:wrap; margin-bottom:12px;">
                                ${['Лицо','Руки','Ноги','Ногти','Зеркало'].map(t => `<div style="background:#F2F2F7; padding:8px 12px; border-radius:10px; font-size:13px; font-weight:600;" onclick="SkinTracker.addText('sk_place', '${t}')">${t}</div>`).join('')}
                            </div>
                            <textarea name="sk_place" class="sk-input" rows="1" placeholder="Где вы были?">${edit ? edit.place : ''}</textarea>
                        </div>
                        ...
                        <div class="sk-form-group">
                            ...
                            <div style="display:flex; gap:8px; flex-wrap:wrap; margin-bottom:12px;">
                                ${['Скука','Тревога','Стресс'].map(t => `<div style="background:#F2F2F7; padding:8px 12px; border-radius:10px; font-size:13px; font-weight:600;" onclick="SkinTracker.addText('sk_before', '${t}')">${t}</div>`).join('')}
                            </div>
                            <textarea name="sk_before" class="sk-input" rows="2" placeholder="Что чувствовали?">${edit ? edit.feeling_before : ''}</textarea>
                        </div>

                        <div class="sk-form-group">
                            <label class="sk-label">Чувства ПОСЛЕ</label>
                            <textarea name="sk_after" class="sk-input" rows="2" placeholder="Что почувствовали потом?">${edit ? edit.feeling_after : ''}</textarea>
                        </div>

                        <div class="sk-form-group">
                            <label class="sk-label">На что переключилась?</label>
                            <textarea name="sk_switch" class="sk-input" rows="1" placeholder="Что помогло отвлечься?">${edit ? edit.switch : ''}</textarea>
                        </div>
