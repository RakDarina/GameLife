const AchievementsModule = {
    config: {
        storageKey: 'GL_Data_Achievements',
        title: 'Достижения',
        placeholder: 'Чем вы гордитесь сегодня? Какую цель закрыли?'
    },
    data: [],
    editingId: null,
    ...
    render: function() {
        ...
        const styles = `
            <style>
                ...
                /* Уникальные стили для шапки достижений */
                .ac-header { display: flex; align-items: center; gap: 15px; margin-bottom: 25px; }
                .ac-back-btn { color: #007AFF; cursor: pointer; font-size: 28px; }
                .ac-title { font-size: 24px; font-weight: 800; flex: 1; text-align: center; margin-right: 40px; color: #1C1C1E; }
        ...
        app.innerHTML = styles + `
            <div class="ls-wrap">
                <div class="ac-header">
                    <span class="material-icons-outlined ac-back-btn" onclick="loadModule('./mental.js')">chevron_left</span>
                    <div class="ac-title">${this.config.title}</div>
                </div>

                <div class="ls-add-btn" onclick="AchievementsModule.openModal('new')">
                    + Добавить запись
                </div>
        ...
        return `
            <div class="ls-modal-bg" onclick="AchievementsModule.editingId=null; AchievementsModule.render()">
                <div class="ls-modal" onclick="event.stopPropagation()">
                    ...
                    ${item ? `<div style="text-align:center; color:#FF3B30; margin-top:15px; font-weight:700; cursor:pointer;" onclick="AchievementsModule.deleteItem(${item.id})">Удалить</div>` : ''}
                </div>
            </div>
        `;
    },
